	...
	<fo:root>
		...
		<fo:page-sequence master-reference="cover">
			<fo:flow flow-name="xsl-region-body">
				...
				<fo:block text-align="left" font-size="10.0pt">
					<xsl:text>Wir liefern Chemikalien AUSSCHLIESSLICH an Schulen, Universitäten sowie gewerbliche Abnehmer.</xsl:text>
				</fo:block>
				<fo:block text-align="left" font-size="10.0pt">
					<xsl:text>Bei Internetbestellungen werden Chemikalien automatisch aus der Bestellung gelöscht wenn der Besteller
diese Voraussetzung nicht erfüllt.</xsl:text>
				</fo:block>
				<fo:block text-align="left" font-size="10.0pt">
					<xsl:text>Bei Neuregisterierung über unsere Internetseite müssen Sie sich, um diese Voraussetzung zu erfüllen, entsprechend anmelden.
Hierzu ist die Angabe von Telefonnummer, Telefaxnummer sowie der Umsatzsteuerident Nr. erforderlich. Wir behalten uns vor diese Angabe nachzuprüfen und
im Zweifelsfall eine Gewerbeanmeldung einzufordern.</xsl:text>
				</fo:block>
				<fo:block text-align="left" font-size="10.0pt">
					<xsl:text>Für Bestellung von Kaliumpermanganat sowie Essigsäureanhydrid ist zusätzliche eine sogenannte Endverbleibserklärung
erforderlich. Das hierzu erforderliche Formular finden Sie in dem Download Bereich. Beide Chemikalien werden ausschliesslich an Schulen und Universitäten
abgegeben.</xsl:text>
				</fo:block>
			</fo:flow>
		</fo:page-sequence>
...
<!--
			<xsl:apply-templates select="document/TableArtikel">
-->
					</xsl:apply-templates>
				</fo:block>
			</fo:flow>
