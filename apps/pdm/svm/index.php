...
if ( $mySysSession->Validity == SysSession::VALIDAPP) {
    ...
    <head>
		...
		-->
        <link rel="stylesheet" href="http://mas.wimtecc.de.local/rsrc/js/jQuery/jquery-ui-1.12.1.custom/jquery-ui.css">
        <script src="http://mas.wimtecc.de.local/rsrc/js/jQuery/jquery-3.2.1.min.js"></script>
        <script src="http://mas.wimtecc.de.local/rsrc/js/jQuery/jquery-ui-1.12.1.custom/jquery-ui.min.js"></script>
        <script src="http://mas.wimtecc.de.local/rsrc/js/jQuery/jquery.ui.autocomplete.html.js"></script>
        <!--
...
-->
    ...
    <div id="centerContainer">
        <div id="centerLeft">
            <div id="Navigator">
				<img src="<?php echo "../../clients/".$mySysSession->ClientId."/apps/".$mySysSession->ApplicationSystemId."/".$mySysSession->ApplicationId."/rsrc/images/mainLogo.png" ; ?>" alt="a fancy logo is still missing ..." onclick="myInfo() ; myDebug() ;" />
				<?php
						...
						if ( $appUser->isGranted( "scr", $module->ModuleName)) {
							...
							foreach ( $screen as $key => $screen) {
								if ( $appUser->isGranted( "scr", $module->ModuleName.".".$screen->ScreenName) || $appUser->isGranted( "scr", $module->ModuleName.".*")) {
									echo "<div class=\"g".(string)($_level+1)."p\" onClick=\"showScreen2( '".$screen->ScreenName."', false) ; return false ;\">" . FTr::tr( $screen->Label) . "</div>\n" ;
								}
							}
							echo "</div>\n" ;
						} else {
