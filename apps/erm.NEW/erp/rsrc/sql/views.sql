...
DROP VIEW IF EXISTS v_AddressSurvey ;
CREATE VIEW v_AddressSurvey AS
	SELECT A.Id, A.AddressNo, CONCAT( A.Name1, " ", A.Name2) AS AddressName, CONCAT( A.ZIP, " ", A.City) AS AddressData
		FROM Address AS A
...
CREATE VIEW v_AddressAddressContactSurvey AS
	SELECT A.Id, A.AddressNo, AC.AddressContactNo, CONCAT( A.Name1, " ", A.Name2) AS AddressName, CONCAT( AC.FirstName, " ", AC.LastName) AS Name, CONCAT( A.ZIP, " ", A.City) AS AddressData
		FROM Address AS A
...
DROP VIEW IF EXISTS v_AddressAddressContactSurvey ;
CREATE VIEW v_AddressAddressContactSurvey AS
	SELECT A.Id, A.AddressNo, CONCAT( A.Name1, " ", A.Name2) AS AddressName, "NO_CONTACT_USED" AS Name
		FROM Address AS A
	ORDER BY A.AddressNo
;
...
CREATE VIEW v_AddressContactSurvey AS
	SELECT AC.Id, AC.AddressNo, AC.AddressContactNo, AC.FirstName, AC.LastName, AC.Phone
...
CREATE VIEW v_CustomerSurvey AS
	SELECT C.Id, C.CustomerNo, CONCAT( C.CustomerName1, " ", C.CustomerName2) AS CustomerName, CONCAT( C.ZIP, " ", C.City, ", ", C.Country) AS CustomerData
		FROM Customer AS C
...
DROP VIEW IF EXISTS v_SupplierSurvey ;
CREATE VIEW v_SupplierSurvey AS
	SELECT S.Id, S.SupplierNo, CONCAT( S.SupplierName1, " ", S.SupplierName2) AS SupplierName, CONCAT( S.ZIP, " ", S.City, ", ", S.Country) AS SupplierData
		FROM Supplier AS S
...
DROP VIEW IF EXISTS v_ProductItemList ;
CREATE VIEW v_ProductGroupItemList AS
	SELECT PGI.Id AS Id, PGI.ProductGroupNo AS ProductGroupNo, PGI.ItemNo AS ItemNo, PGI.CompProductGroupNo AS CompProductGroupNo, PG.ProductGroupName AS ProductGroupName, PGI.CompArticleGroupNo AS CompArticleGroupNo, AG.ArticleGroupName AS ArticleGroupName, PGI.CompArticleNo AS CompArticleNo, A.ArticleDescription1 AS ArticleDescription1
		FROM ProductGroupItem AS PGI
...
;
DROP VIEW IF EXISTS v_ArticleGroupItemList ;
CREATE VIEW v_ArticleGroupItemList AS
	SELECT AGI.Id AS Id, AGI.ArticleGroupNo AS ArticleGroupNo, AGI.ItemNo AS ItemNo, AGI.CompProductGroupNo AS CompProductGroupNo, PG.ProductGroupName AS ProductGroupName, AGI.CompArticleGroupNo AS CompArticleGroupNo, AG.ArticleGroupName AS ArticleGroupName, AGI.CompArticleNo AS CompArticleNo, A.ArticleDescription1 AS ArticleDescription1
		FROM ArticleGroupItem AS AGI
