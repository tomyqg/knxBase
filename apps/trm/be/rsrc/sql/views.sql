...
DROP VIEW IF EXISTS v_DocumentSurvey ;
CREATE VIEW v_DocumentSurvey AS
	SELECT D.Id, D.DocumentNo, DIDC.Key AS  DecClassKey, DIDC.Value AS DecimalClass, DIAC.Key AS ABCClassKey, DIAC.Value AS ABCClass, D.Title
		FROM Document AS D
		LEFT JOIN DocumentInfo AS DIDC on DIDC.Category = 'DECClass' AND DIDC.Key = D.DecimalClass
		LEFT JOIN DocumentInfo AS DIAC ON DIAC.Category = 'ABCClass' AND DIAC.Key = D.ABCClass
;
...
SELECT TABLE_NAME
  ...
  AND NOT EXISTS (
      SELECT * from AuthObject where AuthObjectId = concat( "dbt_", TABLE_NAME, "_*")) ;
