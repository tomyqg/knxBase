...
void	disasMsg( FILE *_oFile, eibHdl *_myEIB, knxMsg *myMsg) {
	...
	fprintf( _oFile, "  From.........: %04x ... %s\n", myMsg->sndAddr, eibIntToHWAddr( myMsg->sndAddr, buffer)) ;
	fprintf( _oFile, "  To...........: %04x ... %s\n", myMsg->rcvAddr, eibIntToGroup( myMsg->rcvAddr, buffer)) ;
	switch ( myMsg->prio) {
...
void	disasHPFunc( FILE *_oFile, eibHdl *_myEIB, knxMsg *myMsg) {
	fprintf( _oFile, "  Priority....: High Priority\n") ;
	switch ( myMsg->tlc) {
	case	0x00	:		// UDP
		break ;
	case	0x01	:		// NDP
