...
class	Customer	extends	AppObject	{
	...
	function	addDep( $_key="", $_id=-1, $_val="", $_reply=null) {
		...
		if ( $objName == "CustomerContact") {
			...
			$myCustomerContact->CustomerContactNo	=	sprintf( "%03d", intval( $myContactNo) + 1) ;
			$myCustomerContact->storeInDb() ;
		} else if ( $objName == "LiefCustomer") {
			$this->_addDepCustomer( "L") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "LiefCustomer", $_reply) ;
		} else if ( $objName == "RechCustomer") {
			$this->_addDepCustomer( "R") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "RechCustomer", $_reply) ;
		} else if ( $objName == "AddCustomer") {
			$this->_addDepCustomer( "A") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "AddCustomer", $_reply) ;
		}
		return $_reply ;
	}
	/**
