 ...
 */
abstract	class	AppObjectERM	extends	AppObject	{
	static	$const ;
	/**
	 *
	 */
...
//			error_log( "................: " . self::$const[ "conf"]) ;
	...
	function	upd( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		if ( $this->LockState == 0) {
			...
			$e	=	new Exception( "AppObjectERM.php::AppObject::upd: The object [" . $this->$keyCol . "] is locked!") ;
			error_log( $e) ;
			throw $e ;
		}
		FDbg::end() ;
	...
	function	del( $_key="", $_id=-1, $_val="", $reply = NULL) {
		...
		if ( $this->LockState == 0) {
			...
			$e	=	new Exception( "AppObjectERM.php::AppObject::del: The object [" . $this->$keyCol . "] is locked!") ;
			error_log( $e) ;
			throw $e ;
		}
		return $this->getXMLString( $_key, $_id, $_val, $reply) ;
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "AppObjectERM.php", "AppObjectERM", "addDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$objName	=	$_val ;
		if ( ! isset( $this->LockState))
		...
		if ( $this->LockState == 0) {
			...
			$e	=	new Exception( 'AppObjectERM.php::AppObject::addDep: the object is locked!') ;
			error_log( $e) ;
			throw $e ;
		}
		$this->getList( $_key, $_id, $objName, $reply) ;
		FDbg::end( 1, "AppObjectERM.php", "AppObjectERM", "addDep( '$_key', $_id, '$_val')") ;
		return $reply ;
	}
	/**
	 *
	...
	function	updDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "AppObjectERM.php", "AppObjectERM", "updDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$elem	=	explode( ".", $_val) ;
		$objName	=	$elem[0] ;
		...
		if ( ! isset( $this->LockState))
			$this->LockState	=	0 ;
		if ( $this->LockState == 0) {
			FDbg::trace( 2, "AppObjectERM.php", "AppObjectERM", "updDep( '$_key', $_id, '$_val')",
	...
	function	delDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "AppObjectERM.php", "AppObjectERM", "delDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$objName	=	$_val ;
		if ( ! isset( $this->LockState))
		...
		if ( $this->LockState == 0) {
			try {
				...
				if ( $tmpObj->setId( $_id)) {
					...
					$e	=	new Exception( "AppObjectERM.php::AppObject::delDep[Id='$_id'] dependent is INVALID !") ;
					error_log( $e) ;
					throw $e ;
				}
			} catch ( Exception $e) {
			...
			$e	=	new Exception( "AppObjectERM.php::AppObject::delDep(...): the object is locked!") ;
			error_log( $e) ;
			throw $e ;
		}
		$this->getList( $_key, $_id, $objName, $reply) ;
		FDbg::end( 1, "AppObjectERM.php", "AppObjectERM", "delDep( '$_key', $_id, '$_val')") ;
		return $reply ;
	}
	/**
	 *
	...
	function	updAnschreiben( $_key, $_id, $_val) {
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::updAnschreiben( '$_key', $_id, '$_val'): begin") ;
		try {
		...
		return $this->getXMLString() ;
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::updAnschreiben( '$_key', $_id, '$_val'): end") ;
	}
	...
	function	decDep( $_key, $_id, $_val) {
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::incDep( '$_key', $_id, '$_val'): begin/end") ;
		...
		if ( $this->LockState == 0) {
			try {
				...
				if ( $tmpObj->setId( $_id)) {
					...
					$e	=	new Exception( 'AppObjectERM.php::AppObject::setSingle::item[Id='.$_id.'] is INVALID !') ;
					error_log( $e) ;
					throw $e ;
				}
			} catch ( Exception $e) {
	...
	function	getDepAsJSON( $_key="", $_id=-1, $_val="", $reply=null) {
		$myKeyCol	=	$this->keyCol ;
		$myKey	=	$this->$myKeyCol ;
		$objName	=	$_val ;
		$tmpObj	=	new $objName() ;
	...
	function	getDocListAsXML( $_key, $_id, $_val) {
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::getDocListAsXML( '$_key', $_id, '$_val'): begin") ;
		$myKeyCol	=	$this->keyCol ;
		$myKey	=	$this->$myKeyCol ;
		$fullPath	=	$this->path->Archive . $this->className . "/" ;
		$ret	=	DocList::getXMLComplete( $fullPath, $myKey, $this->url->Archive."$this->className/") ;
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::getDocListAsXML( '$_key', $_id, '$_val'): end") ;
		return $ret ;
	}
	/**
	 * methods: internal
	...
	function	_newFrom( $_srcObjClass, $_srcObjKey, $_trgObjKey="", $_where="", $_nsStart="000000", $_nsEnd="899999") {
		FDbg::begin( 1, "AppObjectERM.php", "AppObjectERM", "_newFrom(...)") ;
		/**
		...
		try {
			...
			if ( $srcObj->isValid()) {
				...
				try {
					...
					$e	=	new exception( "AppObjectERM.php::AppObject::_newFrom(...): error during _setTexte/_setAnschreiben") ;
					error_log( $e) ;
				}
				FDbg::trace( 1, FDbg::mdTrcInfo1, "AppObjectERM.php", "AppObjectERM", "_newFrom(...)", "preparing source item for copy") ;
				...
				FDbg::trace( 1, FDbg::mdTrcInfo1, "AppObjectERM.php", "AppObjectERM", "_newFrom(...)", "preparing target item for copy") ;
				$trgItemObjClass        =       $this->className . "Item" ;
				$trgItemObj     =       new $trgItemObjClass() ;
				FDbg::trace( 1, FDbg::mdTrcInfo1, "AppObjectERM.php", "AppObjectERM", "_newFrom(...)", "starting to copy items") ;
	...
	function	_stepDep( $_key, $_id, $_val, $_step) {
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::_stepDep( '$_key', $_id, '$_val', $_step): begin") ;
		$depObj	=	explode( ".", $_val) ;
		$objName	=	$depObj[0] ;
		$attrName	=	$depObj[1] ;
		FDbg::dumpL( 0x02000000, "AppObjectERM.php::AppObject::_stepDep(...): objName = $objName, attrName = $attrName") ;
		if ( ! isset( $this->LockState))
			$this->LockState	=	0 ;
		if ( $this->LockState == 0) {
			try {
		...
		$ret	=	"<" . $this->className . ">\n" .
					$this->getDepAsXML( $_key, $_id, $objName) .
					"</" . $this->className . ">\n" ;
		FDbg::dumpL( 0x01000000, "AppObjectERM.php::AppObject::incDepSingle( '$_key', $_id, '$_val', $_step): end") ;
		return $ret ;
	}
	/**
	 *
