...
class	CustomerData	extends Page	{
	...
	function	__construct( $_parent=null, $_lang="de", $_country="de") {
		...
		if ( self::$myCustomer->isValid() === true) {
	...
	function	go( $_myWebPage, $_prodGrNo="", $_artGrNo="", $_articleNo="", $_tmplName="") {
		...
		switch ( $_GET['action']) {
		case	"show"	:
			$xmldoc	=	$this->runCustomerShow( $_myWebPage->WebPageNo, $_prodGrNo, $_artGrNo, $_articleNo, $_tmplName) ;
			break ;
		case	"update"	:
			$xmldoc	=	$this->runCustomerUpdate( $_myWebPage->WebPageNo, $_prodGrNo, $_artGrNo, $_articleNo, $_tmplName) ;
			break ;
		default	:
			$xmldoc	=	$this->runCustomerShow( $_myWebPage->WebPageNo, $_prodGrNo, $_artGrNo, $_articleNo, $_tmplName) ;
			break ;
...
error_log( $xmldoc->saveXML( $startNode)) ;
	 ...
	 * @return unknown
	 */
	function	runCustomerShow( $_webPageNo, $_prodGrNo="", $_artGrNo="", $_articleNo="", $_tmplName="") {
		...
		if ( self::$myCustomer != null) {
			...
			$customerContactNode	=	self::$myCustomerContact->_exportXML( $xmldoc, $customerNode) ;
		} else {
			$customerNode	=	$contentNode->appendChild( $xmldoc->createElement( "Customer")) ;
		}
		FDbg::end() ;
		return $xmldoc ;
	}
	 ...
	 * @return unknown
	 */
	function	runCustomerUpdate( $_webPageNo, $_prodGrNo="", $_artGrNo="", $_articleNo="", $_tmplName="") {
		FDbg::begin( 1, basename( __FILE__), __CLASS__."{Page}", __METHOD__."( '$_webPageNo', '$_prodGrNo', '$_artGrNo', '$_articleNo', '$_tmplName')") ;
		/**
		 *
		 */
	...
	function	runMyCartRFQ( $_webPageNo, $_tmplName="") {
		 ...
		 */
		FDbg::trace( 1, FDbg::mdTrcInfo1, basename( __FILE__), __CLASS__."{Page}", __METHOD__."( '$_webPageNo', '$_tmplName'): try-ing") ;
		self::$myShopSession	=	new ShopSession() ;
		$buffer	=	$this->_getMyCart( $_webPageNo, $_tmplName="") ;
		FDbg::end() ;
		return $buffer ;
	}
	/**
