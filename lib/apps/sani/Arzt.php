 ...
 * @subpackage Arzt
 */
class	Arzt	extends	AppObject	{
	public	$Rights	=	0x00000000 ;
	/**
	 *
	 */
	function	__construct( $_myArztNr="", $_className="Arzt", $_keyCol="ERPNr", $_db="def") {
	 ...
	 *
	 */
	function	setKey( $_myArztNr) {
		parent::setKey( $_myArztNr) ;
//		if ( ! $this->_valid) {
//			$cloudArzt	=	new CloudArzt( $_myArztNr) ;
//			$this->copyFrom( $cloudArzt) ;
//		}
		return $this->_valid ;
	}
	...
	function	add( $_key="", $_id=-1, $_val="", $reply = NULL) {
		...
		$myArzt	=	new Arzt() ;
		if ( $myArzt->first( "LENGTH(ERPNr) = 12", "ERPNr DESC")) {
			$this->getFromPostL() ;
			$this->ERPNr	=	sprintf( "%012d", intval( $myArzt->ERPNr) + 1) ;
			$this->Remark	=	"" ;
			$this->storeInDb() ;
		} else {
			$this->getFromPostL() ;
  ...
  /**
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		switch ( $objName) {
		case	"Arzt"	:
			$myObj	=	new FDbObject( "Arzt", "ArztNr", "def", "v_ArztSurvey") ;
			if ( isset( $_POST['StartRow'])) {
				$myObj->setPage( intval( $_POST['StartRow']), intval( $_POST['RowCount']), $_POST['step']) ;
			}
			$myQuery	=	$myObj->getQueryObj( "Select") ;
			$filter1	=	"ArztNr like '{$sCrit}%' OR Name like '%{$sCrit}%' " ;
			$filter2	=	"" ;
