 ...
 * @subpackage HMV_PA
 */
class	HMV_PA	extends	AppObject	{
	...
	function	__construct( $_myHMVNr="") {
		...
		parent::__construct( "HMV_PA", "HMVNr", "cloud") ;
	...
	function	add( $_key="", $_id=-1, $_val="", $reply = NULL) {
		...
		$myHMV_PA	=	new HMV_PA() ;
		if ( $myHMV_PA->first( "IKNr DESC", "LENGTH(IKNr) = 9")) {
			$this->getFromPostL() ;
			$this->IKNr	=	sprintf( "%08d", intval( $myHMV_PA->IKNr) + 1) ;
			$this->Tax	=	1 ;
			$this->Remark	=	"" ;
			$this->storeInDb() ;
		} else {
			throw new FException( basename( __FILE__), __CLASS__, __METHOD__."( '$_key', $_id, '$_val')",
					"object[".$this->cacheName."], HMV_PA invalid after creation!'") ;
		}
		FDbg::end() ;
	...
	function	_upd( $_key="", $_id=-1, $_val="", $_reply=null) {
		FDbg::begin( 1, "HMV_PA.php", "HMV_PA", "_upd()") ;
		parent::upd( $_key, $_id, $_val, $_replyl) ;
		$this->_addRem( FTr::tr( "HMV_PA updated")) ;
		FDbg::end() ;
	}
	/**
	 *
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		$objName	=	$_val ;
		if ( $objName == "HMV_PAAdresse") {
			$myHMV_PAAdresse	=	new HMV_PAAdresse() ;
			$myHMV_PAAdresse->IKNr	=	$this->IKNr ;
			$myHMV_PAAdresse->newHMV_PAAdresse() ;
			$myHMV_PAAdresse->getFromPostL() ;
			$myHMV_PAAdresse->updateInDb() ;
			$reply->message	=	FTr::tr( "Contact succesfully added!") ;
			$this->getList( $_key, $_id, $objName, $reply) ;
		} else if ( $objName == "LiefHMV_PA") {
			$this->_addDepHMV_PA( "L") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "LiefHMV_PA", $reply) ;
		} else if ( $objName == "RechHMV_PA") {
			$this->_addDepHMV_PA( "R") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "RechHMV_PA", $reply) ;
		} else if ( $objName == "AddHMV_PA") {
			$this->_addDepHMV_PA( "A") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "AddHMV_PA", $reply) ;
		}
		FDbg::end() ;
		return $reply ;
	}
	...
	function	_addDep( $_key="", $_id=-1, $_val="") {
		...
		if ( $objName == "HMV_PAAdresse") {
			...
			$myHMV_PAAdresse->getFromPostL() ;
			$myHMV_PAAdresse->updateInDb() ;
			return $myHMV_PAAdresse->HMV_PAAdresseNo ;
		} else if ( $objName == "LiefHMV_PA") {
			return $this->_addDepHMV_PA( "L") ;
		} else if ( $objName == "RechHMV_PA") {
			return $this->_addDepHMV_PA( "R") ;
		} else if ( $objName == "AddHMV_PA") {
			return $this->_addDepHMV_PA( "A") ;
		}
	}
	/**
	 * buche
	 ...
	 *
	 */
	function	newHMV_PA( $_nsStart="000000", $_nsEnd="899999") {
		FDbg::dumpL( 0x00000001, "HMV_PA.php::HMV_PA::newHMV_PA( $_nsStart, $_nsEnd):") ;
	...
	/**
	 *	Funktion:	identify HMV_PA
	 *
	 *	Versucht irgendwie den HMV_PAn per Password zu identifizieren
	 *	Zuerst wird versucht, den HMV_PAn per HMV_PAnNr zu finden.
	 *	Falls keine HMV_PAnnumemr existiert koennte es die E-Mail Adresse sein.
	 *	Wenn HMV_PAndaten existieren, dann muss geprueft werden, ob das Password
	 *	stimmt.
	 */

	/**
	 * create a new 'HMV_PA' from a form sending POST data
	 */
	function	newHMV_PAFromPOST() {
		$this->getFromPost() ;
		$myCuContact	=	new HMV_PAAdresse() ;
		$myCuContact->getFromPost() ;
		if ( $this->HMV_PAName1 == "") {
			$this->HMV_PAName1	=	$myCuContact->FirstName . " " . $myCuContact->Name ;
		}
		/**
		 *
		 */
		if ( strlen( $this->HMV_PAName1) < 8) {
			self::$err['_IHMV_PAName1']	=	true ;
		}
		...
		if ( strlen( $this->eMail) < 10) {
			...
			$this->newHMV_PA( "900000", "999999") ;
			$this->getPassword() ;
			$myCuContact->IKNr	=	$this->IKNr ;
			$myCuContact->newHMV_PAAdresse() ;
		}
	}
	...
	function	getDepAsXML( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		case	"LiefHMV_PA"	:
			...
			$reply->replyData	=	str_replace( "HMV_PA>", "LiefHMV_PA>", $myHMV_PA->getAsXML()) ;
			break ;
		case	"RechHMV_PA"	:
			$myHMV_PA	=	new HMV_PA() ;
			if ( $_id == -1) {
			...
			$reply->replyData	=	str_replace( "HMV_PA>", "RechHMV_PA>", $myHMV_PA->getAsXML()) ;
			break ;
		case	"AddHMV_PA"	:
			$myHMV_PA	=	new HMV_PA() ;
			if ( $_id == -1) {
			...
			$reply->replyData	=	str_replace( "HMV_PA>", "AddHMV_PA>", $myHMV_PA->getAsXML()) ;
			break ;
		default	:
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		switch ( $objName) {
		case	"HMV_PA"	:
			$myObj	=	new FDbObject( "HMV_PA", "IKNr", "cloud", "v_HMV_PASurvey") ;
			...
			break ;
		case	"HMV_EP"	:
			$myObj	=	new FDbObject( "HMV_EP", "Id", "cloud", "v_HMV_EPSurvey") ;				// no specific object we need here
			if ( isset( $_POST['StartRow'])) {
				$myObj->setPage( intval( $_POST['StartRow']), intval( $_POST['RowCount']), $_POST['step']) ;
			}
