 ...
 * @subpackage Filiale
 */
class	Filiale	extends	AppObject	{
	...
	function	__construct( $_myERPNr="") {
		...
		parent::__construct( "Filiale", "ERPNr") ;
		$this->Rights	=	0x00000001 ;
		if ( strlen( $_myERPNr) > 0) {
			try {
error_log( "-----------___>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> getting .......................::") ;
				$this->setKey( $_myERPNr) ;
			} catch ( Exception $e) {
				throw $e ;
			}
		} else {
	 ...
	 *
	 */
	function	setFilialeNr( $_myERPNr) {
		$this->ERPNr	=	$_myERPNr ;
		$this->reload() ;
		return $this->_valid ;
	}
	...
	function	add( $_key="", $_id=-1, $_val="", $reply = NULL) {
		...
		if ( $_val != "") {
			...
			if ( $myFiliale->first( "LENGTH(ERPNr) = 12", "ERPNr DESC")) {
				$this->getFromPostL() ;
				$this->ERPNr	=	sprintf( "%012d", intval( $myFiliale->ERPNr) + 1) ;
				$this->storeInDb() ;
			} else {
				$this->getFromPostL() ;
				$this->ERPNr	=	sprintf( "%012d", intval( $myFiliale->ERPNr) + 1) ;
				...
				throw new FException( basename( __FILE__), __CLASS__, __METHOD__."( '$_key', $_id, '$_val')",
						"object[".$this->cacheName."], Filiale invalid after creation!'") ;
			}
		}
		FDbg::end() ;
	...
	function	_upd( $_key="", $_id=-1, $_val="", $_reply=null) {
		FDbg::begin( 1, "Filiale.php", "Filiale", "_upd()") ;
		parent::_upd( $_key, $_id, $_val, $_reply) ;
		$this->_addRem( FTr::tr( "Filiale updated " . ( $this->lastUpdateList != "" ? "\n" : "") . str_replace( ",", "\n", $this->lastUpdateList) . "")) ;
		FDbg::end() ;
	}
	/**
	 *
    ...
    function    fetchForNew() {
        $this->FilialeName1   =   $_POST['_IFilialeName1'] ;
        $this->FilialeName2   =   $_POST['_IFilialeName2'] ;
	...
	function	getDepAsXML( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		case	"Filiale"	:
			...
			if ( $_id == -1) {
				$myFiliale->Id	=	-1 ;
				$myFiliale->ERPNr	=	$this->ERPNr ;
				$myFiliale->FilialeNr	=	$this->FilialeNr ;
			} else {
				$myFiliale->setId( $_id) ;
			}
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		switch ( $objName) {
		case	"Filiale"	:
			$myObj	=	new FDbObject( "Filiale", "ERPNr", "def", "v_FilialeSurvey") ;
			...
			break ;
		case	"Kasse"	:
			$myObj	=	new FDbObject( "Kasse", "Id", "def", "v_KasseSurvey") ;				// no specific object we need here
			if ( isset( $_POST['StartRow'])) {
				$myObj->setPage( intval( $_POST['StartRow']), intval( $_POST['RowCount']), $_POST['step']) ;
			}
			$_customerNoCrit	=	$sCrit ;
			$filter1	=	"( FilialeERPNr = '" . $this->ERPNr . "') " ;
			/**
			 *
			 */
			$myQuery	=	$myObj->getQueryObj( "Select") ;
	...
	function	acList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		if ( isset( $_REQUEST['term']))
			$sCrit	=	$_REQUEST['term'] ;
		$myFiliale	=	new Filiale() ;
		$myFiliale->setIterCond( "FirmaERPNr like '%" . $this->__getAppConfig()->fallback->FirmaERPNr . "%' ") ;
		$il0	=	0 ;
		foreach ( $myFiliale as $filiale) {
			if ( $il0 < 50) {
				switch ( $_val) {
				case	3	:
					$a_json_row["id"]		=	$filiale->Id ;
					$a_json_row["value"]	=	$filiale->IKNr ;
					$a_json_row["label"]	=	$filiale->IKNr . ", " . $filiale->Name1 ;
				...
				default	:
					...
					$a_json_row["kundeNr"]	=	$firma->FilialeNr ;
