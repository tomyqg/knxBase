 ...
 * @subpackage HMV_UG
 */
class	HMV_UG	extends	AppObject	{
	...
	function	__construct( $_myHMVNr="") {
		...
		parent::__construct( "HMV_UG", "HMVNr", "cloud") ;
	...
	function	add( $_key="", $_id=-1, $_val="", $reply = NULL) {
		...
		$myHMV_UG	=	new HMV_UG() ;
		if ( $myHMV_UG->first( "IKNr DESC", "LENGTH(IKNr) = 9")) {
			$this->getFromPostL() ;
			$this->IKNr	=	sprintf( "%08d", intval( $myHMV_UG->IKNr) + 1) ;
			$this->Tax	=	1 ;
			$this->Remark	=	"" ;
			$this->storeInDb() ;
		} else {
			throw new FException( basename( __FILE__), __CLASS__, __METHOD__."( '$_key', $_id, '$_val')",
					"object[".$this->cacheName."], HMV_UG invalid after creation!'") ;
		}
		FDbg::end() ;
	...
	function	_upd( $_key="", $_id=-1, $_val="", $_reply=null) {
		FDbg::begin( 1, "HMV_UG.php", "HMV_UG", "_upd()") ;
		parent::upd( $_key, $_id, $_val, $_replyl) ;
		$this->_addRem( FTr::tr( "HMV_UG updated")) ;
		FDbg::end() ;
	}
	/**
	 *
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		$objName	=	$_val ;
		if ( $objName == "HMV_UGAdresse") {
			$myHMV_UGAdresse	=	new HMV_UGAdresse() ;
			$myHMV_UGAdresse->IKNr	=	$this->IKNr ;
			$myHMV_UGAdresse->newHMV_UGAdresse() ;
			$myHMV_UGAdresse->getFromPostL() ;
			$myHMV_UGAdresse->updateInDb() ;
			$reply->message	=	FTr::tr( "Contact succesfully added!") ;
			$this->getList( $_key, $_id, $objName, $reply) ;
		} else if ( $objName == "LiefHMV_UG") {
			$this->_addDepHMV_UG( "L") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "LiefHMV_UG", $reply) ;
		} else if ( $objName == "RechHMV_UG") {
			$this->_addDepHMV_UG( "R") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "RechHMV_UG", $reply) ;
		} else if ( $objName == "AddHMV_UG") {
			$this->_addDepHMV_UG( "A") ;
			$ret	=	$this->getTableDepAsXML( $_key, $_id, "AddHMV_UG", $reply) ;
		}
		FDbg::end() ;
		return $reply ;
	}
	...
	function	_addDep( $_key="", $_id=-1, $_val="") {
		...
		if ( $objName == "HMV_UGAdresse") {
			...
			$myHMV_UGAdresse->getFromPostL() ;
			$myHMV_UGAdresse->updateInDb() ;
			return $myHMV_UGAdresse->HMV_UGAdresseNo ;
		} else if ( $objName == "LiefHMV_UG") {
			return $this->_addDepHMV_UG( "L") ;
		} else if ( $objName == "RechHMV_UG") {
			return $this->_addDepHMV_UG( "R") ;
		} else if ( $objName == "AddHMV_UG") {
			return $this->_addDepHMV_UG( "A") ;
		}
	}
	/**
	 * buche
	 ...
	 *
	 */
	function	newHMV_UG( $_nsStart="000000", $_nsEnd="899999") {
		FDbg::dumpL( 0x00000001, "HMV_UG.php::HMV_UG::newHMV_UG( $_nsStart, $_nsEnd):") ;
	...
	/**
	 *	Funktion:	identify HMV_UG
	 *
	 *	Versucht irgendwie den HMV_UGn per Password zu identifizieren
	 *	Zuerst wird versucht, den HMV_UGn per HMV_UGnNr zu finden.
	 *	Falls keine HMV_UGnnumemr existiert koennte es die E-Mail Adresse sein.
	 *	Wenn HMV_UGndaten existieren, dann muss geprueft werden, ob das Password
	 *	stimmt.
	 */

	/**
	 * create a new 'HMV_UG' from a form sending POST data
	 */
	function	newHMV_UGFromPOST() {
		$this->getFromPost() ;
		$myCuContact	=	new HMV_UGAdresse() ;
		$myCuContact->getFromPost() ;
		if ( $this->HMV_UGName1 == "") {
			$this->HMV_UGName1	=	$myCuContact->FirstName . " " . $myCuContact->Name ;
		}
		/**
		 *
		 */
		if ( strlen( $this->HMV_UGName1) < 8) {
			self::$err['_IHMV_UGName1']	=	true ;
		}
		...
		if ( strlen( $this->eMail) < 10) {
			...
			$this->newHMV_UG( "900000", "999999") ;
			$this->getPassword() ;
			$myCuContact->IKNr	=	$this->IKNr ;
			$myCuContact->newHMV_UGAdresse() ;
		}
	}
	...
	function	getDepAsXML( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		case	"LiefHMV_UG"	:
			...
			$reply->replyData	=	str_replace( "HMV_UG>", "LiefHMV_UG>", $myHMV_UG->getAsXML()) ;
			break ;
		case	"RechHMV_UG"	:
			$myHMV_UG	=	new HMV_UG() ;
			if ( $_id == -1) {
			...
			$reply->replyData	=	str_replace( "HMV_UG>", "RechHMV_UG>", $myHMV_UG->getAsXML()) ;
			break ;
		case	"AddHMV_UG"	:
			$myHMV_UG	=	new HMV_UG() ;
			if ( $_id == -1) {
			...
			$reply->replyData	=	str_replace( "HMV_UG>", "AddHMV_UG>", $myHMV_UG->getAsXML()) ;
			break ;
		default	:
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		switch ( $objName) {
		case	"HMV_UG"	:
			$myObj	=	new FDbObject( "HMV_UG", "HMVNr", "cloud", "v_HMV_UGSurvey") ;
			...
			break ;
		case	"HMV_PA"	:
			$myObj	=	new FDbObject( "HMV_PA", "Id", "cloud", "v_HMV_PASurvey") ;				// no specific object we need here
			if ( isset( $_POST['StartRow'])) {
				$myObj->setPage( intval( $_POST['StartRow']), intval( $_POST['RowCount']), $_POST['step']) ;
			}
