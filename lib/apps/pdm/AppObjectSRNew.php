 ...
 */
class	AppObjectSRNew	extends	AppObject	{

	public	$mySupp ;
	public	$mySuppContact ;
	...
	function	__construct( $_className, $_keyColName) {
		...
		$this->mySupp	=	new Lief() ;
		$this->mySuppContact	=	new LiefKontakt() ;
	}
	function	reload() {
		$this->fetchFromDb() ;
		if ( $this->_valid) {
			try {
				$this->mySupp->setLiefNr( $this->SuppNo) ;
				$this->mySuppContact->setLiefKontaktNr( $this->SuppNo, $this->SuppContactNo) ;
			} catch( Exception $e) {
...
//		$attr	=	$elem[1] ;
	...
	function	updPrices( $_key="", $_id=-1, $_val="") {
		...
		foreach ( $actItem as $key => $item) {
			$myEKPreisR->fetchFromDbWhere( "WHERE LiefNr = $this->SuppNo AND ArtikelNr = '$actItem->ArtikelNr' AND KalkBasis > 0 ") ;
			if ( $myEKPreisR->_valid) {
				$actItem->LiefArtNr	=	$myEKPreisR->LiefArtNr ;
				$myArtikelEKPreis->setIterCond( "LiefNr = $this->SuppNo AND LiefArtNr = '$myEKPreisR->LiefArtNr' "
	...
	function	setLiefFromLKId( $_key, $_kkId, $_val) {
		...
		if ( $tmpLiefKontakt->_valid) {
			try {
				$this->SuppNo	=	$tmpLiefKontakt->LiefNr ;
				$this->SuppContactNo	=	$tmpLiefKontakt->LiefKontaktNr ;
				$this->updateInDb() ;
				$this->reload() ;
			} catch( Exception $e) {
	 ...
	 * @return void
	 */
	function	_setTexte( $_key="", $_id=-1, $_lfSuppNo="") {
		try {
			$myTexte	=	new SysTexte( $this->className."Prefix", $this->SuppNo, $this->mySupp->Sprache) ;
			$this->Prefix	=	$myTexte->Volltext ;
			$this->updateColInDb( "Prefix") ;
			$myTexte	=	new SysTexte( $this->className."Postfix", $this->SuppNo, $this->mySupp->Sprache) ;
	...
	function	_setAnschreiben( $_key="", $_id=-1, $_val="") {
		try {
			$myTexte	=	new SysTexte( $this->className."EMail", $this->SuppNo, $this->mySupp->Sprache) ;
	...
	function	newLief( $_key, $_val, $_id) {
		try {
			$newLief	=	new Lief() ;
			$newLief->add( '', '', 0) ;
			$this->SuppNo	=	$newLief->LiefNr ;
			$this->SuppContactNo	=	$newLief->_addLiefKontakt() ;
			$this->updateInDb() ;
			$this->reload() ;
		} catch ( Exception $e) {
			throw $e ;
...
//			$ordBy	=	"ORDER BY " . self::$rOrdMode[ $_POST[ '_SOrdMode']] ;
	...
	function	getLiefAsXML() {
		$ret	=	"" ;
		$ret	.=	'<LiefAdr>' ;
		$ret	.=	$this->mySupp->getXMLF() ;
		$ret	.=	$this->mySuppContact->getXMLF() ;
		$ret	.=	'</LiefAdr>' ;
		return $ret ;
	}
}