 ...
 * 2. add the main flow
 * 3. add additional flows
 * 4. add the frame(s) with reference to the flow to each master page
 * 5. add character formats
 * 6. add paragraph formats (with reference to the standard character format for
 *    this paragraph
 * 7. add text to the flow
 * 
 ...
 * what's required
 */
require_once( "pkgs/platform/FError.php") ;
require_once( "pkgs/platform/FDbg.php") ;
define('FPDF_FONTPATH','pkgs/fpdf/font/');
require_once( "pkgs/fpdf/fpdf.php") ;

require_once( "BDocFlow.php") ;
require_once( "BDocFrame.php") ;
require_once( "BDocCharFmt.php") ;
require_once( "BDocParaFmt.php") ;
require_once( "BDocFrame.php") ;
require_once( "BDocPage.php") ;
...
class	BDoc	extends	EISSCoreObject	{
	...
	function	__construct( $_copy=false) {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::__constructor()") ;
	...
	function	setSize( $_docSize) {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setSize()") ;
	...
	function	setType( $_docType) {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setType()") ;
	...
	function	setSided( $_docSided) {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setSided()") ;
	 ...
	 */
	function	setupDoc() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupDoc()") ;
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupDoc(), docSided=%d", $this->docSided) ;
		switch ( $this->docSize) {
	 ...
	 */
	function	begin() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::begin()") ;
	...
	function	end( $_pdfName='') {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::end()") ;
	 ...
	 */
	function	newPage() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::newPage()") ;
	 ...
	 */
	function	setupPage() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupPage()") ;
	}
	...
	function	setupFirstPage() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupFirstPage()") ;
	}
	...
	function	setupMidPage() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupMidPage()") ;
	}
	...
	function	setupLastPage() {
		FDbg::dumpL( 0x01000000, "dbg: BDoc::setupLastPage()") ;
	}
	...
	function	addPage( $_page) {
		FDbg::dumpL( 0x01000000, "BDoc::addPage(): name='%s'", $_page->getName()) ;
	...
	function	addFrame( $_frm) {
		FDbg::dumpL( 0x01000000, "BDoc::addFrame(): name='%s'", $_frm->getName()) ;
	...
	function	addFlow( $_flow) {
		FDbg::dumpL( 0x01000000, "BDoc::addFlow(): name='%s'", $_flow->getName()) ;
	...
	function	_debug() {
		if ( FDbg::enabled()) {
			FDbg::dump( "Document::_debug") ;
			FDbg::dump( "Nr. of Pages: %d", count( $this->myPages)) ;
			foreach ( $this->myPages as $ndx => $page) {
				$page->_debug() ;
			}
			reset( $this->myFrames) ;
			FDbg::dump( "Nr. of Flows: %d", count( $this->myFlows)) ;
			FDbg::dump( "Nr. of Frames: %d", count( $this->myFrames)) ;
			FDbg::dump( "Document own attributes") ;
			FDbg::dump( " => docSize=%d", $this->docSize) ;
			FDbg::dump( " => docType=%d", $this->docType) ;
			FDbg::dump( " => docSided=%", $this->docSided) ;
		}
	}
