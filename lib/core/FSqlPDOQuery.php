<?php
/**
 * FSqlPDOQuery.php	-	Database independent query builder.
 *
 ...
 */
/**
 * FDb implements a simple database connection for PDO databases
 *
 * @package 	wapLib
 * @subpackage	Core
 */
 ...
 *		mssql		experimental
 */
class	FSqlPDOQuery		extends	FSqlQuery	{
    function	__construct( $_table, $_prefix="", $_as="") {
        FDbg::begin( 1, basename( __FILE__), __CLASS__, __METHOD__."( '$_table', '$_prefix', '$_as')") ;
 ...
 *		mssql		experimental
 */
class	FSqlPDOSelect	extends	FSqlPDOQuery		{
    function	__construct( $_table, $_prefix="", $_as="") {
        FDbg::begin( 1, basename( __FILE__), "FSqlPDOSelect", "__construct( '$_table', '$_prefix', '$_as')") ;
        parent::__construct( $_table, $_prefix, $_as) ;
        FDbg::end() ;
    }
     ...
     */
    function	getQuery() {
        FDbg::begin( 1, basename( __FILE__), "FSqlSelect", "getQuery()") ;
        if ( isset( $this->pdoSelect) || isset( $this->mssqlSelect)) {
            $buffer	=	$this->pdoSelect ;
        } else {
            $buffer	=	"" ;
            $buffer	.=	"SELECT " ;
        ...
        FDbg::trace( 2, FDbg::mdTrcInfo1, basename( __FILE__), "FSqlSelect", "getQuery()", $buffer) ;
        FDbg::end() ;
        return $buffer ;
    }
    ...
    function	getCountQuery() {
        FDbg::begin( 1, basename( __FILE__), "FSqlSelect", "getCountQuery()") ;
        $buffer =   "" ;
        if ( isset( $this->pdoSelect) || isset( $this->mssqlSelect)) {
//            $buffer	=	$this->pdoCount ;
        } else {
...
//			$buffer	.=	$this->_getGroup() . " " ;
        ...
        FDbg::trace( 2, FDbg::mdTrcInfo1, basename( __FILE__), "FSqlSelect", "getCountQuery()", $buffer) ;
        FDbg::end() ;
        return $buffer ;
    }
    ...
    function	getSumQuery( $_sumcolumn) {
        FDbg::begin( 1, basename( __FILE__), "FSqlSelect", "getSumQuery()") ;
        $buffer =   "" ;
        if ( isset( $this->pdoSelect) || isset( $this->mssqlSelect)) {
//            $buffer	=	$this->pdoCount ;
        } else {
...
//			$buffer	.=	$this->_getGroup() . " " ;
        ...
        FDbg::trace( 2, FDbg::mdTrcInfo1, basename( __FILE__), "FSqlSelect", "getSumQuery()", $buffer) ;
        FDbg::end() ;
        return $buffer ;
    }
 ...
 *
 */
class	FSqlPDOJoin	extends	FSqlPDOQuery		{
    private	static	$code	=	"" ;
    private	$parent	=	null ;
 ...
 *
 */
class	FSqlPDOInsert	extends	FSqlPDOQuery		{
    function	__construct( $_table, $_prefix="", $_as="") {
        parent::__construct( $_table, $_prefix, $_as) ;
    }
    function	getQuery() {
 ...
 *
 */
class	FSqlPDOUpdate	extends	FSqlPDOQuery		{

    /**
     * FSqlPDOUpdate constructor.
     * @param $_table
 ...
 *		mssql		open
 */
class	FSqlPDODelete	extends	FSqlPDOQuery		{

    /**
     * FSqlPDODelete constructor.
     * @param $_table
     * @param string $_prefix
     * @param string $_as
     */
 ...
 *		mssql		tested
 */
class	FSqlPDOStructure		extends	FSqlPDOQuery		{

    /**
     * FSqlPDOStructure constructor.
     * @param $_table
     * @param string $_prefix
     * @param string $_as
     */
