 ...
 * File			Rev.			Function
 * ----------------------------------------------------------------------------
 * FDbg			*				Instrumentalizing Debugger
...
class	FDb {
	...
	private static  $dbNumSpaceStart        =       array() ;
	private static  $dbNumSpaceEnd  =       array() ;
	...
	private function __construct( $_db="def") {
		if ( ! isset( self::$db[$_db])) {
			if ( self::$driver[$_db] == "mysql") {
				...
				if ( count( $parts) == 1) {
					self::$db[$_db]	=	mysqli_connect( self::$dbHost[$_db], self::$dbUser[$_db], self::$dbPasswd[$_db], self::$dbName[$_db]) ;
				} else {
					self::$db[$_db]	=	mysqli_connect( $parts[0], self::$dbUser[$_db], self::$dbPasswd[$_db], self::$dbName[$_db], intval( $parts[1]?$parts[1]:"")) ;
				}
				if ( self::$db[$_db] !== false) {
					mysqli_set_charset( self::$db[$_db], "utf8") ;
				} else {
					error_log( "problems during establishment of db-connection " . self::$dbHost[$_db] . " ... " .  self::$dbUser[$_db] . " ... " .  self::$dbPasswd[$_db] . " ... " .  self::$dbName[$_db]) ;
					die() ;
				}
...
//		error_log( __FILE__ . "::" . __LINE__ . " ... " . $myQuery) ;
		if ( self::$driver[$_db] == "mysql") {
...
//    }
	...
	static	function	rowCount( $_db="def") {
		FDbg::begin( 1, basename( __FILE__), __CLASS__, __METHOD__."( '$_db')") ;
		$rowCount   =   -1 ;
		if ( !isset( self::$inst[$_db])) {
			self::$inst[$_db]	=	new self( $_db) ;
		}
...
//            $rowCount    =   mysqli_affected_rows( self::$db[$_db]) ;
		...
		FDbg::end( "Number of rows := " . $rowCount) ;
		return $rowCount ;
	}
	...
	public	static	function	getCount( $_query, $_db="def") {
		FDbg::begin( 1,basename( __FILE__), __CLASS__, __METHOD__."( <FSqlQuery>, '$_db')") ;
		if ( !isset( self::$inst[$_db])) {
			self::$inst[$_db]	=	new self( $_db) ;
			self::setNumSpace() ;
			self::setEncoding() ;
		...
		FDbg::trace( 1, FDbg::mdTrcInfo1, basename( __FILE__), __CLASS__, __METHOD__."( <FSqlQuery>, '$_db')", "query := '$query' on database '".self::$driver[$_db]." ..... ".$_db) ;
		$sqlResult  =   null ;
		if ( self::$driver[$_db] == "mysql") {
			FDbg::trace(1, FDbg::mdTrcInfo1, basename(__FILE__), __CLASS__, __METHOD__ . "( <FSqlQuery>, '$_db')", "query := '$query' on database '" . self::$driver[$_db] . "");
			$sqlResult = mysqli_query(self::$db[$_db], $query);
			FDbg::trace(1, FDbg::mdTrcInfo1, basename(__FILE__), __CLASS__, __METHOD__ . "( <FSqlQuery>, '$_db')", "query := '$query' on database '" . self::$driver[$_db] . "");
		} else if ( self::$driver[$_db] == "pdo_mysql") {
			FDbg::trace( 1, FDbg::mdTrcInfo1, basename( __FILE__), __CLASS__, __METHOD__."( <FSqlQuery>, '$_db')", "query := '$query' on database '".self::$driver[$_db]."") ;
...
//			mssql_select_db( self::$dbName[$_db]) ;
		...
		FDbg::trace( 1, FDbg::mdTrcInfo1, basename( __FILE__), __CLASS__, __METHOD__."( <FSqlQuery>, '$_db')", "result := '$result'") ;
		FDbg::end() ;
		return ( $count) ;
	}
...
//			mssql_select_db( self::$dbName[$_db]) ;
	...
	static	function	registerDb( $_dbHost, $_dbUser, $_dbPasswd, $_dbName, $_alias="def", $_driver="mysql", $_prefix="", $_numSpaceStart="000000", $_numSpaceEnd="999999") {
		error_log( __FILE__ . ": " . $_dbHost . ":" . $_dbUser . ":" . $_dbPasswd . ":" . $_dbName . ":" . $_alias) ;
