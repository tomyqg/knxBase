<?php
/**
 * CodesToPrint.php - Class definition
 *  Domain:
 *  	- administrative
 * 	CodesToPrint references:
 * 		- n/a
 *  CodesToPrint is referenced by:
 *  	- TrailerType
 *  	- Calculation (twice!)
 *
 ...
 * @version 0.1
 * @package CodesToPrintCalc
 */
/**
 * CodesToPrint - User-Level Class for usage by the application
 *
 ...
 * @subpackage Classes
 */
class	CodesToPrint	extends	AppObjectCore	{
	/*
	...
	function	__construct() {
		parent::__construct( "CodesToPrint", "Id") ;
	}
	/**
...
//					$newItem->Domain	=	$this->__getUser()->Domain ;
	...
	function	updDep( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		if ( isset( $elem[1]))
			...
			$attrName	=	"" ;
		FDbg::trace( 2, "CodesToPrint.php", "CodesToPrint", "updDep( '$_key', $_id, '$_val')",
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		...
		case	""	:
			...
			$filter	.=	"(" ;
			$filter	.=	"( CodesToPrintId like '%" . $_searchCrit . "%' OR ProtocolNo like '%" . $_searchCrit . "%' ) " ;
			$filter	.=	")" ;
			/**
			 *
			 */
			$myObj	=	new FDbObject( "CodesToPrint", "Id", "def", "v_CodesToPrintSurvey") ;				// no specific object we need here
			if ( isset( $_POST['StartRow'])) {
				$myObj->setPage( intval( $_POST['StartRow']), intval( $_POST['RowCount']), $_POST['step']) ;
			}
			$myQuery	=	$myObj->getQueryObj( "Select") ;
			$myQuery->addWhere( [$filter]) ;
			$myQuery->addOrder( ["CodesToPrintId"]) ;
