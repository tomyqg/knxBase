<?php
/**
 * Authority.php - Class definition
 *  Domain:
 *  	- administrative 
 * 	Authority references:
 * 		- n/a
 *  Authority is referenced by:
 *  	- TrailerType
 *  	- Calculation (twice!)
 * 
 ...
 * @version 0.1
 * @package AuthorityCalc
 */
/**
 * Authority - User-Level Class for usage by the application
 *
 ...
 * @subpackage Classes
 */
class	Authority	extends	BCObject	{
	/*
	...
	function	__construct() {
		parent::__construct( "Authority", "AuthorityId") ;
//		parent::setIdKey( "br_key") ;
	...
	function	getXMLComplete( $_key="", $_id=-1, $_val="") {
		...
		$ret	.=	$this->getTableDepAsXML( $_key, $_id, "Authority") ;
		return $ret ;
	}
	/**
	 * (non-PHPdoc)
...
//					$newItem->Domain	=	$this->__getUser()->Domain ;
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Authority.php", "Authority", "addDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		switch( $_val) {
			default	:
	...
	function	updDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Authority.php", "Authority", "updDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$elem	=	explode( ".", $_val) ;
		$objName	=	$elem[0] ;
		if ( isset( $elem[1]))
			...
			$attrName	=	"" ;
		FDbg::trace( 2, "Authority.php", "Authority", "updDep( '$_key', $_id, '$_val')",
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Authority.php", "Authority", "getList( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
