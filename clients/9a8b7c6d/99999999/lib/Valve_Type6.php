 ...
 * @subpackage Classes
 */
class	Valve_Type6	extends	Valve	{
	/*
...
//		$reply->replyMediaType	=	Reply::mediaImgJpg ;
		...
		imagestring( $im, 5, 0, 0, "Valve_Type6", $gridColor);
		...
		$myPoints->setIterOrder( "ORDER BY X ") ;
		$il0	=	0 ;
		foreach ( $myPoints as $key => $obj) {
			if ( $il0 > 0) {
				...
				FDbg::trace( 2, "Valve_Type3.php", "Valve_Type3", "drawing ...", "\n"
					...
					.	"    Y := " . $obj->Y . "") ;
				if ( $il0 == 3) {
					imageline( $im, 25 + $lastX * 25, 175 - $lastY * 15, 25 + $obj->X * 25, 175 - $obj->Y / 6.4 * 15, $gridColor) ;
