<?php
/**
 * CylinderType.php - Class definition
 *  Domain:
 *  	- administrative 
 * 	CylinderType references:
 * 		- n/a
 *  CylinderType is referenced by:
 *  	- TrailerType
 *  	- Calculation (twice!)
 * 
 ...
 * @version 0.1
 * @package CylinderTypeCalc
 */
/**
 * CylinderType - User-Level Class for usage by the application
 *
 ...
 * @subpackage Classes
 */
class	CylinderType	extends	BCObject	{
	/*
	...
	function	__construct() {
		parent::__construct( "CylinderType", "CylinderTypeId") ;
//		parent::setIdKey( "br_key") ;
	...
	function	getXMLComplete( $_key="", $_id=-1, $_val="") {
		...
		$ret	.=	$this->getTableDepAsXML( $_key, $_id, "CylinderType") ;
		return $ret ;
	}
	/**
	 * (non-PHPdoc)
...
//					$newItem->Domain	=	$this->__getUser()->Domain ;
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "CylinderType.php", "CylinderType", "addDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		switch( $_val) {
			default	:
	...
	function	updDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "CylinderType.php", "CylinderType", "updDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$elem	=	explode( ".", $_val) ;
		$objName	=	$elem[0] ;
		if ( isset( $elem[1]))
			...
			$attrName	=	"" ;
		FDbg::trace( 2, "CylinderType.php", "CylinderType", "updDep( '$_key', $_id, '$_val')",
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "CylinderType.php", "CylinderType", "getList( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
