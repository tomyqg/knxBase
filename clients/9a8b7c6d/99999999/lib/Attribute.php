<?php
/**
 * Attribute.php - Class definition
 *  Domain:
 *  	- administrative 
 ...
 * @version 0.1
 * @package AttributeCalc
 */
/**
 * Attribute - User-Level Class for usage by the application
 *
 ...
 * @subpackage Classes
 */
class	Attribute	extends	BCObject	{
	/*
...
//		parent::setIdKey( "br_key") ;
	...
	function	getXMLComplete( $_key="", $_id=-1, $_val="") {
		...
		$ret	.=	$this->getTableDepAsXML( $_key, $_id, "Attribute") ;
		return $ret ;
	}
	/**
	 * (non-PHPdoc)
...
//					$newItem->Domain	=	$this->__getUser()->Domain ;
	...
	function	addDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Attribute.php", "Attribute", "addDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		switch( $_val) {
			default	:
	...
	function	updDep( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Attribute.php", "Attribute", "updDep( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		$elem	=	explode( ".", $_val) ;
		$objName	=	$elem[0] ;
		if ( isset( $elem[1]))
			...
			$attrName	=	"" ;
		FDbg::trace( 2, "Attribute.php", "Attribute", "updDep( '$_key', $_id, '$_val')",
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "Attribute.php", "Attribute", "getList( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
			$reply	=	new Reply( __class__, $this->className) ;
		...
		switch ( $objName) {
			case	""	:	
				...
				$filter	.=	"(" ;
				$filter	.=	"( C.AttributeId like '%" . $_idCrit . "%' OR C.Description like '%" . $_descriptionCrit . "%' ) " ;
				$filter	.=	")" ;
				$myObj	=	new FDbObject( "", "") ;				// no specific object we need here
				$myObj->addCol( "Id", "var") ;
				$myObj->addCol( "AttributeId", "var") ;
				...
				$reply->replyData	=	$myObj->tableFromDb( " ",
										...
										"Attribute",
										"Attribute",
										"C.Id, C.AttributeId, C.Description ") ;
				break ;
			case	"Assessment"	:
				$tmpObj	=	new $objName() ;
				$tmpObj->addCol( "Id", "int") ;
//				$tmpObj->setId( $_id) ;
				$reply->replyData	=	$tmpObj->tableFromDb( " ", "", "AttributeId = '$this->AttributeId' ") ;
				break ;
		}
		FDbg::end() ;
		return $reply ;
