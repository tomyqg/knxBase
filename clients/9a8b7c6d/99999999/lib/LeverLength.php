<?php
/**
 * LeverLength.php - Class definition
 *  Domain:
 *  	- administrative 
 * 	LeverLength references:
 * 		- n/a
 *  LeverLength is referenced by:
 *  	- TrailerType
 *  	- Calculation (twice!)
 * 
 ...
 * @version 0.1
 * @package LeverLengthCalc
 */
/**
 * LeverLength - User-Level Class for usage by the application
 *
 ...
 * @subpackage Classes
 */
class	LeverLength	extends	BCObject	{
	/*
	...
	function	__construct() {
		parent::__construct( "LeverLength", "Id") ;
//		parent::setIdKey( "h_key") ;
	...
	function	getXMLComplete( $_key="", $_id=-1, $_val="") {
		...
		$ret	.=	$this->getTableDepAsXML( $_key, $_id, "LeverLength") ;
		return $ret ;
	}
	/**
	 * (non-PHPdoc)
		...
		switch ( $_val) {
			default	:
				if ( $_id >= 0) {
					...
					$newItem	=	new $_val ;
					$newItem->LeverLengthId	=	$this->LeverLengthId ;
	...
	function	getList( $_key="", $_id=-1, $_val="", $reply=null) {
		FDbg::begin( 1, "LeverLength.php", "LeverLength", "getListAsXML( '$_key', $_id, '$_val')") ;
		if ( $reply == null)
