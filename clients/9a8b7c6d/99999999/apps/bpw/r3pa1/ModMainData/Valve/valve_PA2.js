...
//	this.delConfDialog	=	"/ModBase/Valve/confValveDel.php" ;
	...
	this.gridValveParameters	=	new wapGrid( this, "gridValveParameters", {
									...
									,	onDataSourceLoadFi:	function( _parent, _xmlData) {
															dBegin( 102, "mainvalve{valve.js}", "wapGrid{gridValveParameters}", "onDataSourceLoadFi( <_parent>, <_xmlData>)") ;
//															this.show() ;
															...
															var	canvasData	=	valveCtx.getImageData( 0, 0, valveGraph.width, valveGraph.height) ;
															...
															for ( var il0=0 ; il0 < rows.length ; il0++) {
																...
																for ( var il1=fromX ; il1 < ( toX * 30); il1++) {
																	var	x	=	il1 / 30 ;
																	var	y	=	a * ( x * x) + b * x + c ;
																	var xd	=	x * 30 ;
																	var	yd	=	200 - y * 20 ;
																	var	index	=	( Math.round( yd) * 300 + Math.round( xd)) * 4 ;
				...
				valveCtx.beginPath() ;
				valveCtx.arc( xd, yd, 5, 0, Math.PI * 2) ;
				valveCtx.stroke() ;

															}
															valveCtx.putImageData( canvasData, 0, 0);
															dTrace( 2, "mainvalve{valve.js}", "wapGrid{gridValveParameters}", "onDataSourceLoadFi( <_parent>, <_xmlData>)", "il0 := " + il0.toString()) ;
															dEnd( 102, "mainvalve{valve.js}", "wapGrid{gridValveParameters}", "onDataSourceLoadFi( <_parent>, <_xmlData>)") ;
														}
									}) ;
	/**
	 * link to this screen
